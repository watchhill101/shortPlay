...
router.put('/:userId', authMiddleware, async (req, res) => {
  try {
    ...
    const user = await User.findByIdAndUpdate(userId, updateData, { new: true, runValidators: true }).select(
      '-password -douyinProfile.accessToken -douyinProfile.refreshToken'
    );
...
router.post('/upload/avatar', authMiddleware, upload.single('avatar'), async (req, res) => {
  try {
    ...
    // 更新用户头像
    const user = await User.findByIdAndUpdate(req.user.id, { avatar: avatarUrl }, { new: true }).select('avatar');
