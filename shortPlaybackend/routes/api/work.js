...
router.get('/:workId', async (req, res) => {
  try {
    ...
    const work = await Work.findById(workId)
      .populate('collectionId', 'title') // 关联查询合集信息
      .lean();
    ...
    const formattedWork = {
      ...work,
      collectionTitle: work.collectionId?.title || '未知合集',
      episodeNumber: work.episodeNumber || 1,
      title: work.title || '未知标题',
    };
...
router.post('/like/:workId', async (req, res) => {
  try {
    ...
    // 根据isLiked状态更新点赞数
    work.likeCount = isLiked ? (work.likeCount || 0) + 1 : Math.max(0, (work.likeCount || 0) - 1);
...
router.post('/collect/:workId', async (req, res) => {
  try {
    ...
    work.commentCount = isCollected ? (work.commentCount || 0) + 1 : Math.max(0, (work.commentCount || 0) - 1);
