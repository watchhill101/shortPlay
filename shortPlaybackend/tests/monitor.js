  ...
  recordMetrics() {
    try {
      ...
      if (process.env.VERBOSE_MONITORING) {
        console.log(
          `📊 ${timestamp} - CPU: ${cpuUsage.toFixed(1)}%, 内存: ${memoryInfo.used.toFixed(1)}MB, 负载: ${loadAvg.toFixed(2)}`
        );
      }
    } catch (error) {
  ...
  getStats() {
    ...
    for (const endpoint in this.metrics.requestCounts) {
      ...
      stats[endpoint] = {
        ...
        errorRate:
          (((this.metrics.errorCounts[endpoint] || 0) / this.metrics.requestCounts[endpoint]) * 100).toFixed(2) + '%',
      };
    }
 ...
 * 启动完整监控
 */
async function startFullMonitoring(duration = 300000) {
  // 默认5分钟
