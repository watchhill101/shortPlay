...
async function customStressTest() {
  ...
  try {
    ...
    const concurrentResult = await tester.concurrentUsersTest(
    ...
    // 3. 负载递增测试
    console.log('\n📈 3. 负载递增测试');
    const rampResult = await tester.loadRampTest(
    ...
    // 4. 错误处理测试
    console.log('\n🔧 4. 错误处理测试');
    const errorTest = await tester.errorRecoveryTest();
    ...
    if (config.testDuration > 60000) {
      ...
      await tester.enduranceTest(
        config.testDuration, 
        Math.floor(config.concurrentUsers / 3)
      );
    }
