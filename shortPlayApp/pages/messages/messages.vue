    ...
    <view class="virtual-list-container" @scroll="onScroll" :scroll-top="scrollTop">
      ...
      <view v-else class="virtual-list">
        ...
        <view class="virtual-viewport" :style="{ transform: `translateY(${offsetY}rpx)` }">
          <view v-for="item in visibleMessages" :key="item.id" class="message-item" @click="openChat(item)">
...
export default {
  ...
  onShow() {
    ...
    this.currentUser = authService.getCurrentUser();
    this.loadConversations();
    if (!this.socketConnected) this.setupSocket();
  },
  onPullDownRefresh() {
    this.refreshData();
  },
  methods: {
    ...
    async loadConversations() {
      const userIdGuard = this.currentUser && (this.currentUser.id || this.currentUser._id);
      if (!userIdGuard) {
        this.messageList = [];
        return;
      }
      ...
      try {
        ...
        if (response.data && response.data.success) {
          ...
          this.messageList = response.data.data.conversations.map(conv => ({
            ...
            lastMessage: conv.lastMessage
              ? conv.lastMessage.fromSelf
                ? `我: ${this.getPreviewText(conv.lastMessage?.messageType, conv.lastMessage?.content)}`
                : this.getPreviewText(conv.lastMessage?.messageType, conv.lastMessage?.content)
              : '暂无消息',
            time: this.formatTime(conv.lastMessage?.timestamp || conv.lastMessage?.createdAt),
          ...
          if (!this.messageList.length) {
            await this.loadFriendsAsConversations();
          }
        } else {
    ...
    async loadFriendsAsConversations() {
      try {
        ...
        if (res.statusCode === 200 && res.data?.success) {
          ...
          this.messageList = friends.map(item => ({
            ...
            lastMessage: '暂无消息',
            time: this.formatTime(item.lastInteractionAt),
            unreadCount: 0,
    ...
    setupSocket() {
      ...
      try {
        ...
        // 收到好友发来的消息
        this.socket.on('receiveFriendMessage', msg => {
          this.handleIncomingMessage(msg);
        });

        // 我方发送成功，也更新到列表顶端
        this.socket.on('friendMessageSent', ({ message }) => {
    ...
    // 处理进入的消息，更新/插入会话到顶部
    handleIncomingMessage(message, fromSelf = false) {
      if (!message) return;
      const userId = this.currentUser?.id || this.currentUser?._id;
      ...
      const preview = this.getPreviewText(message.messageType, message.content);

      const idx = this.messageList.findIndex(x => x.friendId?.toString() === friendId?.toString());
      if (idx >= 0) {
        const conv = this.messageList[idx];
    ...
    getPreviewText(type, content) {
      ...
      if (!type || type === 'text') {
        return this.truncateText(text, 36);
      }
      return map[type] || '[消息]';
    },

    truncateText(s, maxLen = 36) {
      if (!s) return '';
    ...
    formatTime(timestamp) {
      ...
      try {
        const ms = Date.parse(timestamp) || Number(timestamp) || 0;
        const time = new Date(ms || timestamp);
        const now = new Date();
        const diff = now - time;
        ...
        } else if (time.toDateString() === now.toDateString()) {
          // 今天
