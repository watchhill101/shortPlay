              ...
              <view class="comment-body">
                ...
                <view class="comment-actions">
                  <view class="comment-action" @click="likeComment(comment._id)">
                    ...
                    <text v-if="comment.likeCount > 0" :class="['comment-action-count', { 'liked': comment.isLiked }]">{{ comment.likeCount }}</text>
                ...
                <view v-if="comment.replies && comment.replies.length > 0" class="reply-list">
                  <view v-for="(reply, replyIndex) in comment.replies" :key="reply._id || `reply_${replyIndex}`" class="reply-item">
...
async function updateCollectionFollowStatus(collectionId: string, isFollowing: boolean) {
  try {
    ...
    if (!safeCollectionId || safeCollectionId === '[object Object]' || safeCollectionId === 'undefined' || safeCollectionId === 'null') {
