...
const updateResult = (name, status, message) => {
  const index = testResults.value.findIndex(r => r.name === name);
  if (index >= 0) {
    testResults.value[index].status = status;
    testResults.value[index].message = message;
    testResults.value[index].time = new Date();
  }
};

const testBackendConnection = async () => {
  const testName = '后端API服务';
  addResult(testName, 'testing', '正在测试...');
  try {
    ...
    if (response.statusCode === 200) {
      updateResult(testName, 'success', '连接成功');
    } else {
      updateResult(testName, 'failed', `HTTP ${response.statusCode}`);
    }
  } catch (error) {
    updateResult(testName, 'failed', error.message || '连接失败');
  }
};
...
const testDatabaseConnection = async () => {
  ...
  try {
    ...
    if (response.data?.data?.services?.mongodb === 'connected') {
      updateResult(testName, 'success', 'MongoDB连接正常');
    } else {
      updateResult(testName, 'failed', 'MongoDB连接异常');
    }
  } catch (error) {
    updateResult(testName, 'failed', error.message || '测试失败');
  }
};
...
const testRedisConnection = async () => {
  ...
  try {
    ...
    if (response.data?.data?.services?.redis === 'connected') {
      updateResult(testName, 'success', 'Redis连接正常');
    } else {
      updateResult(testName, 'failed', 'Redis连接异常');
    }
  } catch (error) {
    updateResult(testName, 'failed', error.message || '测试失败');
  }
...
const testAllConnections = async () => {
  testResults.value = [];
  await Promise.all([testBackendConnection(), testDatabaseConnection(), testRedisConnection()]);
};
