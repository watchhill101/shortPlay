...
const testLoadMessages = async () => {
  if (!currentUserId.value || !friendId.value) {
    testResult.value = '请先输入用户ID和好友ID';
    return;
  }

  try {
    testResult.value = '正在加载聊天记录...';

    const response = await request.get(`/api/chat/history/${friendId.value}`, {
      params: {
    ...
    if (response.data && response.data.success) {
      ...
      testResult.value = `成功加载 ${messageList.value.length} 条聊天记录`;
    } else {
      messageList.value = [];
      testResult.value = `加载失败: ${response.data?.message || '未知错误'}`;
    }
  } catch (error) {
...
const testDeleteMessages = async () => {
  if (!currentUserId.value || !friendId.value) {
    testResult.value = '请先输入用户ID和好友ID';
    return;
  }
  ...
  try {
    testResult.value = '正在删除聊天记录...';

    const response = await request.delete(`/api/chat/conversation/${friendId.value}`, {
      data: {
        userId: currentUserId.value,
      },
      headers: { Authorization: `Bearer ${tokenManager.getAccessToken()}` },
    ...
    if (response.data && response.data.success) {
      testResult.value = `删除成功: 删除了 ${response.data.data.deletedCount} 条记录`;
    } else {
      testResult.value = `删除失败: ${response.data?.message || '未知错误'}`;
    }
  } catch (error) {
...
const testLoadAfterDelete = async () => {
  testResult.value = '删除后重新加载中...';
  await testLoadMessages();
  testResult.value += ' (删除后重新加载完成)';
};

const clearLocalStorage = () => {
  try {
    const conversationId = [currentUserId.value, friendId.value].sort().join('_');
    const oldConversationId = `${currentUserId.value}_${friendId.value}`;
    ...
    const filteredConversations = conversations.filter(
      conv => conv.id !== conversationId && conv.id !== oldConversationId
    );
    uni.setStorageSync('conversations', filteredConversations);

    testResult.value = '本地存储已清空';
  } catch (error) {
    testResult.value = `清空本地存储失败: ${error.message}`;
  }
};
</script>
