	...
	methods: {
		...
		async loadRequests() {
			...
			try {
				...
				if (response.data && response.data.success) {
					this.requestList = response.data.data.requests;
					
					// 更新计数
					if (this.activeTab === 'received') {
						this.receivedCount = this.requestList.filter(req => req.status === 'pending').length;
					} else {
						this.sentCount = this.requestList.filter(req => req.status === 'pending').length;
					}
				} else {
		...
		async handleRequest(requestId, action) {
			...
			uni.showModal({
				...
				success: async (res) => {
					if (res.confirm) {
						await this.doHandleRequest(requestId, action);
					}
				}
			});
		},
		...
		// 执行处理申请 - 连接真实后端
		async doHandleRequest(requestId, action) {
			try {
