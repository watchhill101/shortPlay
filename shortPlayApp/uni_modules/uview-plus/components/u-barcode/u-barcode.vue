<template>
  <view class="u-barcode" v-if="calcSizeDone">
    ...
    <image
      ...
      mode="aspectFit"
    />
    <view v-else class="error-container" :style="{ width: canvasWidth + 'px', height: canvasHeight + 'px' }">
...
export default {
  ...
  methods: {
    ...
    encodeEAN52(data, format) {
      ...
      const pattern = format === 'EAN5' ? patterns[checksum % 10] : patterns[parseInt(data) % 4];
    ...
    drawBarcode(ctx, barcodeData, options) {
      ...
      const moduleWidth = Math.max(
        1,
        (this.canvasWidth - marginLeft - (options.marginRight || 10)) / barcodeData.length,
      );
