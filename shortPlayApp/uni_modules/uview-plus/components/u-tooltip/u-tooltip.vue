            ...
            <template v-if="!$slots['content']">
              ...
              <block v-for="(item, index) in buttons" :key="index">
                <view
                  ...
                  hover-class="u-tooltip__wrapper__popup__list__btn--hover"
                >
                  <text class="u-tooltip__wrapper__popup__list__btn__text" @tap="btnClickHandler(index)">{{
                    item
                  }}</text>
                </view>
    ...
    getTooltipStyle() {
      ...
      if (this.direction === 'left') {
        ...
        if (this.tooltipInfo.width / 2 > this.textInfo.left + this.textInfo.width / 2 - this.screenGap) {
          ...
          sysInfo.windowWidth - this.textInfo.right + this.textInfo.width / 2 - this.screenGap
        ) {
          this.indicatorStyle = {};
