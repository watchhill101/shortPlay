  ...
  methods: {
              ...
              if (!this.hasSel) {
                ...
                if (this.arWidth && this.arHeight) {
                  let areaWidth =
                    ...
                    areaHeight =
                      this.arHeight.indexOf('rpx') >= 0
                        ? parseInt(this.arHeight) * this.pxRatio
                        : parseInt(this.arHeight);
    ...
    chooseImage(index = undefined, params = undefined, data = undefined) {
      if (params) {
        ...
        expWidth &&
          (this.expWidth = expWidth.indexOf('rpx') >= 0 ? parseInt(expWidth) * this.pxRatio : parseInt(expWidth));
        expHeight &&
          (this.expHeight = expHeight.indexOf('rpx') >= 0 ? parseInt(expHeight) * this.pxRatio : parseInt(expHeight));
      ...
      if (touch1) {
        ...
        if (this.letChangeSize) {
          ...
          if (Math.abs(x - left) < controlPointSize && Math.abs(y - top) < controlPointSize) {
            ...
            Math.abs(x - (left + width)) < controlPointSize &&
            Math.abs(y - (top + height)) < controlPointSize
          ) {
